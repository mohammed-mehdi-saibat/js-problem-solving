...
let destinationsArray = spaceData.destinations;
//---------------------------EXTRACT DESTINATIONS ARRAY--------------------
...
function countTotalDestinations() {
  let output = `The total number of destinations is: ${destinationsArray.length}`;
  return output;
}
...
function getAvailableDestinations() {
  ...
  for (let i = 0; i < destinationsArray.length; i++) {
    if (destinationsArray[i].available == true) {
      availableDestinations.push(destinationsArray[i]);
    }
  }
